...
namespace SipSorcery.Demo
{
    class Program
    {
        static string USERNAME = "30241827";
        static string PASSWORD = "TgD7mRRhxmaY";
        static string DOMAIN = "34.210.91.112";
        ...
        private static readonly NAudio.Wave.WaveFormat _waveFormat = new NAudio.Wave.WaveFormat(8000, 16, 1);
        private static WaveFileWriter _waveFile = new WaveFileWriter("outputAudio.wav", _waveFormat);
        private static IAudioFingerprintRepository _repo = null;
        private static readonly string _baseURL = "https://localhost:44380/audiofingerprint/";
        ...
        static async Task DialNumber()
        {
            ...
            {
                //StartTimer();
                Thread.Sleep(8000); // wait till IVR speaks a complete message.
                Console.WriteLine("Enter digits one after another");
                var foundTrack = await MakeAPICall("get-matching-audio?audioFileName=copyOutputAudio.wav", "GET");
                var audioTrackName = foundTrack.Content.ReadAsStringAsync().Result;
                if (audioTrackName == "Webex meeting audio")
                {
                    Console.WriteLine("Match Found");
                    string[] meetingNumber = new string[] { "1", "2", "6", "0", "5", "7", "0", "5", "2", "5" };
                    for(var i = 0;i< meetingNumber.Length;i++)
                    {
                        Console.WriteLine(meetingNumber[i]);
                        await userAgent.SendDtmf(byte.Parse(meetingNumber[i]));
                    }
        ...
        //{
           //File.Copy("outputAudio.wav", @"C:\Users\harshadk.MUMBAI1\source\repos\AudioFingerprinting.Demo\SipSorcery.Demo\bin\Debug\netcoreapp3.1\copyOutputAudio.wav", true);
           //await MakeAPICall("generate-audio-fingerprints", "POST");
        //}
        ...
        //    // Resolving
        //    _repo = container.Resolve<IAudioFingerprintRepository>();
        //}
        ...
        private static async Task<HttpResponseMessage> MakeAPICall(string relativeURL, string httpType)
        {
            if(httpType == "GET")
            {
                using (var client = new HttpClient())
                {
                    var response = await client.GetAsync(_baseURL + relativeURL);// "get-matching-audio?audioFileName=WelcomeABC_Alice.wav");
                    return response;
                }
            }
            ...
            if(httpType == "POST")
            {
                var content = new AudioDetails
                {
                    AudioFileName = "WelcomeABC_Alice.wav"
                };
                var jsonString = JsonConvert.SerializeObject(content);

                using (var client = new HttpClient())
                {
                    var response = await client.PostAsync(_baseURL + relativeURL, new StringContent(jsonString, Encoding.Default, "application/json"));
                    return response;
                }
            }
